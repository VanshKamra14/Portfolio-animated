...
const Project = ({thumbnail, title, link, description, languageIcons}: Props) => {
  ...
  const xRotation = useTransform(yspring, [-0.5, 0.5],
    ["15deg", "-15deg"]
  );
  const yRotation = useTransform(xspring, [-0.5, 0.5],
    ["-15deg", "15deg"]
  );
  ...
  return (
    ...
    items-stretch w-full max-w-[428.4px]
    ...
    style={{
      ...
      <Image 
      src={thumbnail} alt={`Thumbnail for ${title}`}
      width={392} height={230} className="w-full h-[230px] object-cover"
      style={{
      ...
      <div className='flex flex-col gap-[11px]'>
        <div className='flex justify-between items-center'>
          ...
          <a href={link.url} className='felx px-[5px] py-[3px]
        ...
        <p className='line-clamp-2 text-sm md:tetx-base'>{description}</p>
